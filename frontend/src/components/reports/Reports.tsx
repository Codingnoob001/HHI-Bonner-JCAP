      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={genderData} cx="50%" cy="50%" labelLine={false} label={({
                name,
                percent
              }) => `${name}: ${(percent * 100).toFixed(0)}%`} outerRadius={80} fill="#8884d8" dataKey="value">
                  {genderData.map((entry, index) => <Cell key={`cell-${index}`} fill={theme === 'dark' ? DARK_COLORS[index] : COLORS[index]} />)}
                </Pie>
                ...
                <Legend formatter={value => <span className="text-gray-800 dark:text-gray-200">
                      {value}
                    </span>} />
              </PieChart>
            </ResponsiveContainer>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={languageData} cx="50%" cy="50%" labelLine={false} label={({
                name,
                percent
              }) => `${name}: ${(percent * 100).toFixed(0)}%`} outerRadius={80} fill="#8884d8" dataKey="value">
                  {languageData.map((entry, index) => <Cell key={`cell-${index}`} fill={theme === 'dark' ? DARK_COLORS[index] : COLORS[index]} />)}
                </Pie>
                <Tooltip contentStyle={{
                backgroundColor: theme === 'dark' ? '#1F2937' : '#FFFFFF',
                borderColor: theme === 'dark' ? '#374151' : '#E5E7EB',
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={healthMetricsData}>
                ...
                <Tooltip />
                <Legend />
                <Line type="monotone" dataKey="avgSystolic" name="Avg. Systolic" stroke="#8884d8" />
                <Line type="monotone" dataKey="avgDiastolic" name="Avg. Diastolic" stroke="#82ca9d" />
                <Line type="monotone" dataKey="avgCholesterol" name="Avg. Cholesterol" stroke="#ffc658" />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
