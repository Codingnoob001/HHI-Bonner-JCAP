...
import { UserIcon, ClipboardIcon, CheckCircleIcon, UserPlusIcon, Trash2, EditIcon, RefreshCw, AlertTriangle } from 'lucide-react';
  ...
  const formatRelativeTime = (dateStr: string, timeStr?: string) => {
    try {
      ...
      if (diffDays === 0) {
        ...
        return timeStr ? `${diffDays} days ago at ${formatTime(timeStr)}` : `${diffDays} days ago`;
      } else {
  ...
  return (
    <div className="space-y-4">
      ...
      {activities.map((activity, index) => (
        <div key={index} className="flex items-start p-3 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-md">
          ...
          {(activity.type !== 'delete' || activity.entity_type !== 'patient') && activity.client_id && (
            <div>
              <Link 
