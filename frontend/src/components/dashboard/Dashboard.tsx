...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        const metrics = await dashboardApi.getMetrics(dateRange.start, dateRange.end);
      ...
      {!loading && !error && metricsData && trendsData && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <StatCard 
            ...
            change={formatPercentage(metricsData.total_patients.change_percentage)} 
          ...
          <StatCard 
            ...
            change={formatPercentage(metricsData.new_patients.change_percentage)} 
            timeframe="from previous period"
          ...
          <StatCard 
            ...
            change={formatPercentage(metricsData.total_visits.change_percentage)} 
            timeframe="from previous period"
          ...
          <StatCard 
            ...
            change={formatPercentage(metricsData.follow_up_compliance.change_percentage)} 
            timeframe="from previous period"
            trend={getTrend(metricsData.follow_up_compliance.change_percentage)} 
            icon={<CheckCircleIcon size={24} />} 
            color={getTrend(metricsData.follow_up_compliance.change_percentage) === 'up' ? 'green' : 'red'} 
