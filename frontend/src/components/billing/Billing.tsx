...
import { SearchIcon, FilterIcon, ChevronDownIcon, ChevronUpIcon, CalendarIcon } from 'lucide-react';
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
          <div className="flex items-center justify-between">
            ...
            <span className={`text-xs px-2 py-1 rounded-full ${financialMetrics.utilizationRate > 75 ? 'bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-200' : 'bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200'}`}>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
          ...
          <p className={`text-2xl font-semibold mt-2 ${financialMetrics.monthlyVariance < 0 ? 'text-red-600' : 'text-green-600'}`}>
          ...
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={monthlyData}>
                ...
                <XAxis dataKey="month" stroke="#9CA3AF" style={{
                fontSize: '12px'
              }} />
                ...
                <Legend />
                <Bar dataKey="budgeted" name="Budgeted" fill={theme === 'dark' ? '#60A5FA' : '#93C5FD'} />
                <Bar dataKey="actual" name="Actual" fill={theme === 'dark' ? '#2563EB' : '#3B82F6'} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
          ...
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={monthlyData}>
                ...
                <Legend />
                <Line type="monotone" dataKey="budgeted" name="Budgeted" stroke="#93c5fd" strokeWidth={2} />
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              {serviceRecords.map(record => <tr key={record.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                  ...
                  <td className={`px-6 py-4 whitespace-nowrap text-sm text-right font-medium ${record.difference < 0 ? 'text-red-600' : 'text-green-600'}`}>
                    {formatCurrency(record.difference)}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${record.reimbursementStatus === 'Pending' ? 'bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200' : 'bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200'}`}>
