...
import { HomeIcon, UsersIcon, CreditCardIcon, BarChartIcon, SettingsIcon, FileTextIcon } from 'lucide-react';
const Sidebar = ({
  collapsed
}) => {
  const navItems = [{
  ...
  return <aside className={`${collapsed ? 'w-16' : 'w-64'} bg-white dark:bg-gray-800 shadow-md transition-all duration-300 ease-in-out overflow-y-auto`}>
      <div className={`flex items-center h-16 px-4 ${collapsed ? 'justify-center' : 'justify-start'}`}>
        {collapsed ? <div className="h-8 w-8 rounded-full bg-blue-600 dark:bg-blue-500 flex items-center justify-center text-white font-bold">
            M
