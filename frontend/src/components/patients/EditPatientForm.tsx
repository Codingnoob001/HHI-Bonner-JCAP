...
const EditPatientForm = ({
  ...
  const [goalsHistory, setGoalsHistory] = useState<PatientGoals[]>([]);
  const [selectedGoalsDate, setSelectedGoalsDate] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    // Fetch all goals history
    const fetchGoalsHistory = async () => {
      try {
        const response = await patientApi.getPatientGoalsHistory(
          patient.client_id,
        );
        setGoalsHistory(response);

        // Default to the latest goals date if available
        if (response.length > 0) {
          setSelectedGoalsDate(response[0].visit_date);
        }
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (const goal of selectedGoals) {
        ...
        const apiKey = Object.entries(GOAL_MAPPING).find(
          ([_, value]) => value === goal,
        )?.[0];
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-8">
          ...
          {goalsHistory.length > 0 && (
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                Edit Goals From Date:
              </label>
              <select
                value={selectedGoalsDate || ""}
                onChange={(e) => {
                  setSelectedGoalsDate(e.target.value);
                  const selectedGoals = goalsHistory.find(
                    (g) => g.visit_date === e.target.value,
                  );
                  if (selectedGoals) {
                    setSelectedGoals(getActiveGoalsFromData(selectedGoals));
                  }
                }}
                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
                {goalsHistory.map((goal) => (
                  <option key={goal.visit_date} value={goal.visit_date}>
                    {new Date(goal.visit_date).toLocaleDateString()}
                    {goal.visit_date === goalsHistory[0].visit_date
                      ? " (Latest)"
                      : ""}
                  </option>
                ))}
